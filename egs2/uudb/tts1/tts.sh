...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        _opts=
        if "${use_emodims}"; then
            _opts+="--use_emodims ${use_emodims} "
            _opts+="--emotion_dims ${emotion_dims} "
        fi
        local/data.sh ${_opts} ${local_data_opts}
    fi
	...
	# Emotion dimensions
	if "${use_emodims}"; then
            log "Stage 2+: Prepare emotion dimensions: data/ -> ${data_feats}/"
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                if [ "${dset}" = "${train_set}" ] || [ "${dset}" = "${valid_set}" ]; then
                    _suf="/org"
                else
		...
		cp data/${dset}/utt2emodim ${data_feats}${_suf}/${dset}/utt2emodim
	    done
	fi
...
if ! "${skip_train}"; then
	...
	if "${use_emodims}"; then
            _opts+="--train_data_path_and_name_and_type ${_train_dir}/utt2emodim,emodims,csv_float "
            _opts+="--valid_data_path_and_name_and_type ${_valid_dir}/utt2emodim,emodims,csv_float "
	fi
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
	    ...
	    if "${use_emodims}"; then
		_ex_opts+="--data_path_and_name_and_type ${_data}/utt2emodim,emodims,csv_float "
	    fi
